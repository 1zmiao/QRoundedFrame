...
列表项目使用 C++ 模型和 SQLite 存储，QML 侧只创建可见区域附近的 delegate。普通模式保留更多热缓存来保证切换速度，低内存模式会减少缓存规模；需要批量任务时优先使用工具页的导入和批量创建能力，不建议把上万行数据一次性展开成普通 QML Item。

独立顶层子窗口会额外创建一套窗口、场景和缓存；如果页面只用于临时配置或局部预览，优先使用页内子窗口。页内子窗口复用主窗口场景，内存开销更低，也避免多顶层窗口频繁创建。
...
1. 复制 `app/cpp/ui_runtime`、`app/cpp/frameless_native`、`app/prebuilt`、`qml`、`resources` 和 `run.py`。
2. 把业务页面放到 `qml/pages`，通用控件放到 `qml/controls`，主题 token 继续走 `qml/core/Theme.qml`。
3. 新增页面时在 C++ runtime 的页面注册表和 QML 导航里加入口；需要 Python 生态时，把业务放到 `app/workers/`，用 JSON stdin/stdout、SQLite、文件队列或本地 IPC 和 C++ UI 通信。
