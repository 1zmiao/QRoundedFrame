...
static int nativeCornerRadiusPx(int radius, QWindow *window) {
    if (radius <= 0)
        return 0;
    const qreal dpr = window ? window->devicePixelRatio() : 1.0;
    return qMax(1, int(std::lround(qreal(radius) * qMax<qreal>(1.0, dpr))));
}
...
static void setGtkFrameExtentsForWindow(QWindow *window, const QMargins &margins) {
    Display *display = sharedXDisplay();
...
#if defined(Q_OS_LINUX) && defined(FRAMELESS_NATIVE_HAS_X11_SHAPE)
    if (m_window)
        setGtkFrameExtentsForWindow(m_window, m_gtkFrameExtents);
#endif
}
...
#else
#if defined(Q_OS_LINUX) && defined(FRAMELESS_NATIVE_HAS_X11_SHAPE)
    if (!m_window || !message || !m_linuxSystemMoveActive)
        return false;
...
void NativeWindowAgent::applyClientSideShadowInputRegion() {
    ...
    const QSize size = m_window->size();
    const int left = m_gtkFrameExtents.left();
    const int top = m_gtkFrameExtents.top();
    const int right = m_gtkFrameExtents.right();
    const int bottom = m_gtkFrameExtents.bottom();
    ...
    XRectangle rect = makeRect(short(left), short(top), unsigned(contentWidth), unsigned(contentHeight));
...
void NativeWindowAgent::applyWindowRegionForNativeSize(int width, int height, bool redraw) {
    ...
    const bool square = m_cornerRadius <= 0
                        || m_window->visibility() == QWindow::Maximized
                        || m_window->visibility() == QWindow::FullScreen;
    const int radius = square ? 0 : qMin(nativeCornerRadiusPx(m_cornerRadius, m_window), qMin(width, height) / 2);
    const QSize size(width, height);
    if (m_lastRegionSize == size && m_lastRegionRadius == radius)
        return;
